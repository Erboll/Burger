...
function App() {
    ...
    setIngredients((prevState) => {
      return prevState.map((inCount) => {
        if (inCount.name === ingName) {
          return { ...inCount, count: inCount.count - 1 };
        }
        return inCount;
      });
    });
  ...
  const getTotalPrice = () => {
    return INGREDIENTS.map((ing) => getCount(ing.name) * getPrice(ing.name));
  };
  const allPrice = getTotalPrice().reduce((acc, value) => acc + value, 0);
          ...
          <div className="img-btn">
            {INGREDIENTS.map((ingredient) => (
              <IngredientComp
                ...
                totalPrice={
                  getCount(ingredient.name) * getPrice(ingredient.name)
                }
                key={Math.random()}
              >
